...
<section id="sidebar"> 
  ...
  <div id="sidebar-nav" style="background-color: black;"> 
    ...
    <ul>
      ...
      <li class="active" ><a href="{{route('advisor_upgrade')}}"><i class="fa fa-thumbs-up"></i> Advisor Upgradation</a></li>
...
<table class="table table-dark">
  ...
  @foreach(App\User::all()->where('role_id_fk','=',3) as $advisor)
    <tr>
      ...
      <td>{{$advisor->email}}</td>
      @if($advisor->advisorrelation->jazzid != null)
      <td>{{$advisor->advisorrelation->jazzid->jazz_transection_id}}</td>
      <td>{{$advisor->advisorrelation->jazzid->contact_no}}</td>
      @else
      <td>{{"Not Given"}}</td>
      <td>{{"Not Given"}}</td>
      @endif
      <td>
      @if($advisor->advisorrelation->is_upgrated==0)
        <form action="{{route('advisor_upgrade_id')}}" method="post">
        @csrf
          <input type="hidden" name="ad_id" value="{{$advisor->id}}">
